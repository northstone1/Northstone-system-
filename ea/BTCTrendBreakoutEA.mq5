//+------------------------------------------------------------------+
//|                                         BTCTrendBreakoutEA.mq5   |
//|      Trend-filtered Donchian breakout EA for BTCUSD (MT5)       |
//+------------------------------------------------------------------+
...
input double InpMaxSpreadPercent    = 0.15;    // Skip entries if spread exceeds this % of price
input int    InpSlippagePoints      = 100;     // Max allowed slippage (points) - crypto moves fast, wider than gold's default
...
int OnInit()
{
   ...
   {
      Print("BTCTrendBreakoutEA: failed to create indicator handle(s)");
      return INIT_FAILED;
   }
...
void OnTick()
{
   ...
   if(buySignal && CountOwnPositions(POSITION_TYPE_BUY) < InpMaxPositions)
   {
      ...
      if(lots > 0)
         trade.Buy(lots, _Symbol, ask, sl, tp, "BTCTrendBreakout buy");
   }
   else if(sellSignal && CountOwnPositions(POSITION_TYPE_SELL) < InpMaxPositions)
   {
      ...
      double lots = CalculateLotSize(slDistance);
      if(lots > 0)
         trade.Sell(lots, _Symbol, bid, sl, tp, "BTCTrendBreakout sell");
   }
}
//+------------------------------------------------------------------+
